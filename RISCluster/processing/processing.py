...
def detector_recstalta(tr, signal_args, detector_args):
    ...
    S_out = np.empty((len(on), int(NFFT/2 + 1) + 1,
                     int(fs*T_seg/(fs*1 - overlap*fs*1) + 1)))
    ...
    for i in range(len(on)):
        ...
        i0 = int(on[i] - fs*4 + random.randint(-fs*1,fs*1))
        # Index2 is T_seg sec after i0:
        i1 = int(i0 + T_seg*fs)
        # Calculate spectrogram and save to file:
        t_spec, f_spec, S = get_specgram(tr, T_seg, fs, i0, i1, NFFT, overlap)
...
def get_datasets(T_seg, NFFT, fs, group_name, overlap):
    ...
    m = 1
    n = int(NFFT/2 + 1) + 1
    o = int(fs*T_seg/(fs*1 - overlap*fs*1) + 1)
...
def get_specgram(tr, T_seg, fs, i0, i1, NFFT, overlap):
...
def mass_data_downloader(savepath, start='20141201', stop='20161201',
    ...
    mdl.download(
        domain,
        restrictions,
        mseed_storage="../Data/MSEED/",
        stationxml_storage="../Data/StationXML/")
